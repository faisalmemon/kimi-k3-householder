...
$4.5\times$ at $C=4$ to $33.8\times$ at $C=64$. The near-linear trend on the
...
\begin{figure}[t]
  ...
  \caption{Amortised speedup (WY build cost spread over 100 applies) vs sequential,
           as a function of $C$ on a log-2 axis. Values rise from $4.5\times$ at
           $C=4$ to $33.8\times$ at $C=64$, proportional to $C$.}
  \label{fig:speedup}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{fp32 precision only.} Production inference typically uses bf16 or
        fp16. \emph{This benchmark now uses bf16 by default}, matching production
        precision and enabling Tensor-Core utilisation. A comparison with fp32
        baselines is left as future work.
\end{enumerate}
